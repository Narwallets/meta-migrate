...
export default class Logic extends BaseLogic {
    ...
    STNEAR_WNEAR_STABLE_POOL_ID: number = 3514
    ...
    async stepTwoAction(amounts: { stnearAmount: string; wnearAmount: string }): Promise<void> {
        ...
        const lpShares: string = this.calcLpSharesFromAmountsForStableStNearWNear(total_shares, pool_amounts, [
            stnearAmount,
            wnearAmount
        ])
        ...
        if (this.isFarmActive) {
            // stake on farm
            actions.push(this.farmStake(lpShares, this.STNEAR_WNEAR_STABLE_POOL_ID))
        }
